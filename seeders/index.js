...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Rey.Schaefer14' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'Qb5yESIb8_pa10x',
        'isDeleted':false,
        'username':'Rey.Schaefer14',
        'email':'Adam.Nienow@yahoo.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Rey.Schaefer14' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Floy.Crona8' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'n2CbapX4DZ5ax4D',
        'isDeleted':false,
        'username':'Floy.Crona8',
        'email':'Lola53@hotmail.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Floy.Crona8' }, userToBeInserted);
    }
